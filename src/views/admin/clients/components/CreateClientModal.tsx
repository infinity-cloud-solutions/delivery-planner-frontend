...
import { CreateClientPayload, MappedClient } from 'types/client';

interface CreateClientModalProps {
  isOpen: boolean;
  onClose: () => void;
  onCreate: (payload: CreateClientPayload) => Promise<void>;
  onClientExistsCheck: (phoneNumber: string) => Promise<MappedClient | null>;
}

const CreateClientModal = ({ isOpen, onClose, onCreate, onClientExistsCheck }: CreateClientModalProps) => {
    ...
    if (clientPhoneNumber.length === 10) {
      ...
      try {
        ...
        if (clientData) {
          ...
          setClientDiscount(String(clientData.clientDiscount ?? 0));
          setNameTouched(true);
